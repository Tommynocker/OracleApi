...
struct OracleController: RouteCollection {
    ...
    func boot(routes: RoutesBuilder) throws {
        ...
        // Angebote mit Status 180,181,182,189
        route.get("allAngebote",use: getAngebote)
    ...
    @Sendable
    func getAngebote(req: Request) async throws -> [Angebote] {
        guard let connection = req.application.storage[OracleStorageKey.self]
        else {
            throw Abort(
                .internalServerError, reason: "Oracle connection not found")
        ...
        let sql = """
            SELECT CREATEDATE, CREATEUSER , ANR, MNR, KTXT, KOMM, ZDESC
            FROM US_BLZ_ANGEBOTE
            """
        ...
        for try await (
            date, createUser, anr, mnr, ktxt, komm, zdesc
        ) in rows.decode(
            (
                Date?, String?, String?, String?, String?, String?, String?
            ).self)
        {
            let angebot = Angebote(
